...
pub fn get_statistics(jrl_dir_path: &PathBuf, entries_to_consider: usize) -> io::Result<()> {
    ...
    for file in recent_entries {
            ...
            match pager::get_next_chunk(&mut lines) {
                Ok(this_chunk) => {
                    ...
                    if prompt_type == PromptQuestionType::Rating {
                        if let Ok(rating) = this_chunk.get_answer()?[0]
                            .get_text()?
                            .as_str()
                            .parse::<f64>()
                        {
        ...
        if let Some(file_rating) = final_file_rating_opt {
            ...
            if best_rating_opt.is_none_or(|r| r < file_rating) {
            ...
            if worst_rating_opt.is_none_or(|r| r > file_rating) {
    ...
    if best_rating_opt.is_none() {
        ...
        if let Some(best_rating) = best_rating_opt {
            println!(
                "{} was your best and worst day with {} rating:",
                best_rating_day, best_rating
            );
        ...
        if let Some(worst_rating) = worst_rating_opt {
            println!(
                "{} was your worst day with {} rating:",
                worst_rating_day, worst_rating
            );
    ...
    if let Some(longest_text_rating) = longest_text_rating_opt {
        println!(
            "{} was your longest input day with {} rating:",
            longest_text_day, longest_text_rating
        );
    ...
    for (word, count) in word_dict.iter() {
        ...
        if word_len > 0 && word_len <= max_word_len && most_common_words[word_len].1 < *count {
            most_common_words[word_len] = (word, *count);
        }
    }
    for (i, word_item) in most_common_words.iter().enumerate().take(max_word_len) {
        let (word, len) = *word_item;
        if len != 0 {
            println!(
                "Most common {} letter word was {}, used {} times",
                i, word, len
            );
        }
    }
